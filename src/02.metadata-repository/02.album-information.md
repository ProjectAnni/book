...
2. 当该曲目的类型为 `Instrumental` 时，主艺术家为对应非 `Instrumental` 版本的 `Vocal` 或 `Composer`。特别地，当该曲目对应非 `Instrumental` 曲目为某曲目的 `Rearrange` 时，主艺术家为对应非 `Instrumental` 版本的 `Vocal` 或 `Arranger`。
...
2. 《Track B》为 `Artist A` 个人单曲中的某一音轨，曲目类型为 `Instrumental`。此时主艺术家可以为 **`Vocal`**（即 `Artist A`），或该 `Instrumental` 的编曲（**`Composer`**）。
3. 《Track C》为 `Artist A` 个人单曲中的某一音轨，曲目类型为 `Instrumental`，且该音轨为 《Track B》 的 `Rearrange`。此时主艺术家可以为 **`Vocal`**（即 `Artist A`），或该 `Instrumental` 的编曲（**`Composer`**），或该 `Instrumental` 的 **`Rearranger`**。
