...
```json
{
  ...
  // 作为 key 的字符串表示该分享允许访问者访问的专辑 UUID
...
音频分发基于音频所在 `Disc` 的 `UUID`、 `disc_id` 和 `track_id`。

获得音频的 `API` 定义为 `GET /{UUID}/{disc_id}/{track_id}`。
...
封面分发基于音频所在专辑或 `Disc` 的 `UUID`。获得封面的 `API` 定义为 `GET /{UUID}/cover`。
...
歌词信息基于音频所在专辑 `Disc` 的 `UUID`、`disc_id` `track_id`。获取歌词信息的 `API` 定义为 `GET /{UUID}/{disc_id}/{track_id}/lyrics`。
...
歌词分发的 `API` 定义为 `GET /{UUID}/{disc_id}/{track_id}/lyric`，可用参数如下：
