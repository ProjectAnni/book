...
响应内容包含当前仓库所有可用的专辑 `catalog` 与专辑 `Catalog` 和 `Disc Catalog` 的映射，如下例：

```json
{
  // 所有可用的专辑 Catalog
  "albums": ["KSLA-0178", "KSLA-0155~9", "TS-0003"],
  // 对含有多张 Disc 的专辑，额外返回专辑 Catalog 与 Disc Catalog 的映射
  // 请求音频和歌词时应使用 Disc catalog，即数组中的 Catalog 请求资源
  // 请求封面时，专辑 Catalog 和 Disc Catalog 都可以使用
  "discs": {
    "KSLA-0155~9": ["KSLA-0155", "KSLA-0156", "KSLA-0157", "KSLA-0158", "KSLA-0159"],
    "TS-0003": ["TS-0003-01", "TS-0003-02"]
  }
}
```
...
```json
{
  ...
  // 作为 key 的字符串表示该分享允许访问者访问的专辑（Disc Catalog）
...
音频分发基于音频所在 `Disc` 的 `catalog` 和 `track_id`。
...
封面分发基于音频所在专辑或 `Disc` 的 `catalog`。获得封面的 `API` 定义为 `GET /{catalog}/cover`。
...
歌词信息基于音频所在专辑 `Disc` 的 `catalog` 和 `track_id`。获取歌词信息的 `API` 定义为 `GET /{catalog}/{track_id}/lyrics`。
...
歌词分发的 `API` 定义为 `GET /{catalog}/{track_id}/lyric`，可用参数如下：
